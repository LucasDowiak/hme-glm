#' Summary Method
#' 
#' Summarize the fitted parameters of the HME
#' 
#' @param obj An S3 object of class "hme"
#' 
#' @param vcv The type of variance-covariance matrix to use for standard errors.
#'            Defaults to the outer product of the gradient (OPG)
#'
#' @param type Select which nodes of the HME to provide summary reports
#' 
#' @export
#' 
summary.hme <- function(obj,
                        vcv=c("OPG", "hessian", "sandwich"),
                        type=c("all", "experts", "gates"))
{
  ...
  if (vcv == "OPG") {
    std_errs <- sqrt(diag(solve(obj$vcv[[vcv]])))
  } else if (vcv == "hessian") {
    stop("Hessian var-cov not implemented")
    std_errs <- sqrt(diag(-solve(obj$vcv[[vcv]])))
  } else {
    stop("Sandwich var-cov not implemented")
    std_errs <- sqrt(diag(obj$vcv[[vcv]]))
  }
  
  
  # the std_errs vector is not broken up as a list of gate or expert nodes
  # so we must do it before summarizing
