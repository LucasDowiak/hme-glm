...
#' 
em_algo <- function(tree, family, posteriors, Y, X, Z,
                    exp.pars, gat.pars)
{
  ...
  for (e in names(exp.pars)) {
    ...
    opt_blk <- optimize_block(Y, X, jpw, family)
  ...
  updated_densities <- napply(names(exp.out), par_to_expert_dens, family,
...
#' 
optimize_block <- function(Y, X, wts, family)
{
  ...
  if (inherits(family, "family")) {
    ...
    out <- glm.fit(x=X, y=Y, weights=wts, family=family)
...
glm_score <- function(expert, expert.pars, family, gate_probs, densities, Y, X)
{
  ...
  # This is where we need to deviate from the theory since we are dealing with
  # a mixture model and the observations need to be weighted by their probability
  # of being generated by a given expert
  dispersion <- if (family$family == "gaussian") {
    sum(wts * eps**2) / df.r
